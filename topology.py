...
class Topology:
    ...
    def write_footer(self,fout) -> None:
    ...
    def write_nucleicacid_nonbondparams(self,fout,type,excl_rule,data):
        print (">> Writing nonbond_params section")
        ##add non-bonded r6 term in sop-sc model for all non-bonded non-native interactions.
        fout.write("\n%s\n"%("[ nonbond_params ]"))
    ...
    def write_moleculetype(self,fout):
        print (">> Writing moleculetype section")
        fout.write("\n%s\n"%("[ moleculetype ]"))
        fout.write("%s\n"%("; name            nrexcl"))
        fout.write("%s\n"%("  Macromolecule   3"))

    def write_protein_atoms(self,fout,type,cgfile,seq,inc_charge):
    ...
    def write_nucleicacid_atoms(self,fout,type,cgfile,seq,inc_charge):
        print (">> Writing atoms section")
        fout.write("\n%s\n"%("[ atoms ]"))
        fout.write("%s\n"%(";nr  type  resnr residue atom  cgnr"))
    ...
    def write_protein_bonds(self,fout,data,func):
    ...
    def write_protein_angles(self,fout,data):
    ...
    def write_protein_dihedrals(self,fout,data,chiral):
    ...
    def write_exclusions(self,fout,data):
        ...
        for i in range(nfiles):
            ...
            if len(self.allatomdata[i].prot.lines) > 0 and self.CGlevel["prot"] in (1,2):
                ...
                with open(prot_topfile,"w+") as ftop:
                    ...
                    self.write_protein_pairs(fout=ftop, data=proc_data_p,excl_rule=excl,charge=charge)
                    self.write_protein_bonds(fout=ftop, data=proc_data_p,func=bond_function)
                    self.write_protein_angles(fout=ftop, data=proc_data_p)
                    self.write_protein_dihedrals(fout=ftop, data=proc_data_p,chiral=CBchiral)
                    self.write_exclusions(fout=ftop,data=proc_data_p)
            ...
            if len(self.allatomdata[i].nucl.lines) > 0 and self.CGlevel["nucl"] in (1,3,5):
                ...
                with open(nucl_topfile,"w+") as ftop:
                    ...
                    self.write_header(fout=ftop,combrule=excl)
                    self.write_nucleicacid_atomtypes(fout=ftop,type=self.CGlevel["nucl"],data=proc_data_n,seq=cgpdb.nucl.seq,rad=rad)
                    self.write_nucleicacid_nonbondparams(fout=ftop,data=proc_data_n,type=self.CGlevel["nucl"],excl_rule=excl)
                    self.write_moleculetype(fout=ftop)
                    self.write_nucleicacid_atoms(fout=ftop,type=self.CGlevel["nucl"],cgfile=cgpdb.pdbfile,seq=cgpdb.nucl.seq,inc_charge=charge.P*(not self.opt.opensmog))
                    self.write_nucleicacid_pairs(fout=ftop, data=proc_data_n,excl_rule=excl,charge=charge)
                    self.write_nucleicacid_bonds(fout=ftop, data=proc_data_n,func=bond_function)
                    self.write_nucleicacid_angles(fout=ftop, data=proc_data_n)
                    self.write_nucleicacid_dihedrals(fout=ftop, data=proc_data_n,chiral=CBchiral)
                    self.write_exclusions(fout=ftop,data=proc_data_n)
                    self.write_footer(fout=ftop)
...
class SOPSC_IDR(Reddy2017):
    ...
    def write_protein_localangreps(self,fout,data,o2n_atn):
        print ("> Using -C6 repulsion for local beads")
        fout.write(";angle based rep temp\n;%5s %5s %5s %5s %5s\n"%("i","j","func","-C06(Rep)","C12 (N/A)"))        
        func=1
        data.Angles()
        ...
        new2old_atn={v:k for k,v in o2n_atn.items()}
...
class Baratam2024(SOPSC_IDR):
    ...
    def write_protein_pairs(self,fout,data,excl_rule,charge):
        ...
        for index in range(len(data.contacts)):
            ...
            for x in range(len(pairs)):
                if "i" in all_atn[pairs[x][0]] or "i" in all_atn[pairs[x][1]]: continue
                checK_pair=[all_atn[pairs[x][0]],all_atn[pairs[x][1]]]
                if checK_pair[0].startswith("CA"): checK_pair[0]=int(checK_pair[0][2:])
                else: checK_pair[0]=int(checK_pair[0][3:])
                if checK_pair[1].startswith("CA"): checK_pair[1]=int(checK_pair[1][2:])
                else: checK_pair[1]=int(checK_pair[1][3:])
                if checK_pair[0]!=checK_pair[1]: continue
                temp_p.append(pairs[x]);temp_c.append(chains[x])
                temp_d.append(dist[x]);temp_w.append(eps[x])
            pairs,chains,dist,eps=temp_p,temp_c,temp_d,temp_w
            del(checK_pair)
